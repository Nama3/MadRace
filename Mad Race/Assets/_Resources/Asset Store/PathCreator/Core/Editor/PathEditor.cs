    ...
    public class PathEditor : Editor {
        ...
        void DrawBezierPathInspector () {
            using (var check = new EditorGUI.ChangeCheckScope ()) {
                ...
                {
                    if (data.showPoints = EditorGUILayout.Foldout(data.showPoints, "Points"))
                    {
                        EditorGUI.indentLevel++;
                        for (var i = 0; i < data.bezierPath.NumPoints; i++)
                            data.bezierPath.SetPoint(i, EditorGUILayout.Vector3Field($"Point {i}", data.bezierPath.GetPoint(i)));
                        EditorGUI.indentLevel--;
                    }
